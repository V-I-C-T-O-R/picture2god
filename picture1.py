...
def onmouse(event, x, y, flags, param):  # 鼠标事件的回调函数
    global ix, iy, drawing, mode,color,text,size,position,color_map,font_map
    global org, img, mosaictmp
    color_map = {
        '灰': '#7d7d7d',
        '黑': '#000',
        'h': '#7d7d7d',
    ...
    font_map = {
        't': r"./fonts/simhei.ttf",
        'd': r"./fonts/simhei.ttf",
        'p': r"./fonts/_H_HelveticaNeue.ttc", #ios数字字体
        'p_ch': r"./fonts/_H_Helvetica.ttc", #ios汉字字体
        'x': r"./fonts/MILT_RG.ttf",
    }
    if event == cv2.EVENT_LBUTTONDOWN:  # 按下左键
        ...
        position = input("输入字体,ios输入“p”,小米输入“x”: ")
        color = input("输入要写入的颜色（输入“h/灰”或“b/黑或bb/白色”）: ")
        text = input("输入要写入的内容,例如(0731) 8520 7777/0731 8886 1979: ")
        size = int(input("请输入字体大小（例如时间输入“ios-38,xiaomi(大-80,小-36)“,时长输入“ios-42,xiaomi-45”,号码输入“ios-82,xiaomi-80”）："))
        # mosaic(img, ix, iy, x, y)  # 马赛克处理
        img = img_inpaint(img, ix, iy, x, y)  # cv2.inpaint调用
        # img = cleanself(img, ix, iy, x, y)  # 平移像素处理
        org = img.copy()
...
def img_inpaint(img, ix, iy, x, y):
    ...
    # cv2.imwrite('img_inpaint.jpg', img)
    return tianzi(img, (ix, iy), text, color_map.get(color), size,pic_path)
...
def cleanself(img, ix, iy, x, y, nsize=1):
    ...
    # cv2.imwrite('cleanself.jpg', img)

    return tianzi(img, (ix, iy), text, color_map.get(color), size,pic_path)


def tianzi(img, site, txt,color,size = 42, path = 'result.jpg'):
    image = Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
    font = ImageFont.truetype(font_map.get(position), size)
    # 创建Draw对象:
    draw = ImageDraw.Draw(image)
    # 输出文字(可以连续写入):
    draw.text(site, txt, font=font, fill=color, spacing=2)
...
if __name__ == '__main__':
    ...
    cv2.resizeWindow("Imageorg", int(width / 3), int(height / 3))
